...
import { Action, Drug, GameMode, Hustler, ItemSlot, ShopItemInfo } from "../types";
import {
  shortString,
...
export interface SystemsInterface {
  ...
  buyItem: (gameId: string, slot: ItemSlot) => Promise<SystemExecuteResult>;
...
export const useSystems = (): SystemsInterface => {
  ...
  const buyItem = useCallback(
    async (gameId: string, slot: ItemSlot) => {
      const { hash, events, parsedEvents } = await executeAndReceipt("shop", "buy_item", [gameId, slot]);
